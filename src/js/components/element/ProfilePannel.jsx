    ...
    return (
        <div className="dropdown">
            <button
                ...
                className={`flex justify-content-center items-center rounded-circle ${menuOpened ? 'show' : ''}`}
            >
            ...
            <div ref={dropdownRef} className={`dropdown-menu to-top dropdown-menu-sm z-10 ${menuOpened ? 'show block' : ''}`}>
                <div className="py-12 px-16 radius-8 bg-primary-50 mb-16 flex items-center justify-between gap-2">
                    <div>
                ...
                <ul className="to-top-list">
                    <li>
                        <Link to={home_url(`/users/${user?.id ?? ''}/view`)} className="dropdown-item text-black px-0 py-8 hover-bg-transparent cursor-pointer hover-text-primary flex items-center gap-3">
                            <User className="icon text-xl" />  {__('My Profile')}</Link>
                    </li>
                    <li>
                        <Link to={home_url('/notifications')} className="dropdown-item text-black px-0 py-8 hover-bg-transparent cursor-pointer hover-text-primary flex items-center gap-3">
                            <MailCheck className="icon text-xl" />  {__('Notifications')}</Link>
                    </li>
                    <li>
                        <Link to={home_url('/settings')} className="dropdown-item text-black px-0 py-8 hover-bg-transparent cursor-pointer hover-text-primary flex items-center gap-3">
                            <Settings className="icon text-xl" />  {__('Setting')}</Link>
                    </li>
                    <li>
                        <button
                            onClick={logout}
                            className="dropdown-item text-black px-0 py-8 hover-bg-transparent cursor-pointer hover-text-danger flex items-center gap-3"
                        >
