...
import Application from './components/AiApp';

jQuery(window).on('elementor:init', function () {
    let panelRoot = null;
    let modal = null;
    let modalRoot = null;
    const TAB_ID = 'sitecore-ai-tab';
    const PANEL_ID = 'sitecore-ai-panel';

    const initAiPanel = () => {
        let el = document.getElementById(PANEL_ID);
        if (!el) {
            const panelContent = document.querySelector('#elementor-panel-content-wrapper');
            if (!panelContent) return;
            el = document.createElement('div');
            el.id = PANEL_ID;
            el.className = 'sitecore-ai-panel-content h-full flex flex-col bg-[#f1f3f5]';
            el.style.display = 'none';
            panelContent.appendChild(el);
        }
        if (!panelRoot) {
            panelRoot = createRoot(el);
            panelRoot.render(<Application />);
        }
        el.style.display = 'flex';
        const elementsView = document.querySelector('#elementor-panel-elements-search-area');
        if (elementsView) elementsView.closest('.elementor-tab-content').style.display = 'none';
        const categories = document.getElementById('elementor-panel-elements-categories');
    ...
    const hideAiPanel = () => {
        ...
        const elementsView = document.querySelector('#elementor-panel-elements-search-area');
        if (elementsView) elementsView.closest('.elementor-tab-content').style.display = 'block';
        const categories = document.getElementById('elementor-panel-elements-categories');
        if (categories) categories.style.display = 'block';
    };

    elementor.on('panel:ready', () => {
        const panelComponent = window.$e.components.get('panel/elements');
        if (panelComponent) {
            panelComponent.addTab(TAB_ID, { title: 'AI Agent', icon: 'eicon-ai' });
        }
        elementor.panel.currentView.on('tab:change', (tab) => {
            if (tab === TAB_ID) initAiPanel();
            else hideAiPanel();
        });
    });
    ...
    const initAiModal = () => {
        ...
        modal = elementor.dialogsManager.createWidget('lightbox', {
            id: 'eai-modal',
            className: 'eai-agent-panel-modal',
            message: `<div id="eai-root"></div>`,
            closeButton: false,
            draggable: false,
            closeOnBackgroundClick: false,
            onShow: () => {
                const container = document.getElementById('eai-root');
                if (container) {
                    container.style.height = '100%';
                    container.parentElement.style.height = '100%';
                    container.parentElement.parentElement.style.top = 0;
                    container.parentElement.parentElement.style.left = 0;
                    container.parentElement.parentElement.style.width = '100%';
                    container.parentElement.parentElement.style.height = '100%';
                }
                if (!modalRoot) modalRoot = createRoot(container);
                modalRoot.render(<Application modal={modal} />);

                const $widget = modal.getElements('widget');
                const $window = modal.getElements('window');
                ...
                $widget.css({
                    ...
                    margin: 0,
                    zIndex: 10000,
                    'pointer-events': 'all', // Re-enable pointer events for the UI itself
    ...
    elementor.on('preview:loaded', () => {
        ...
        if (elementor.$previewContents) {
            ...
            const previewBody = elementor.$previewContents.find('body')[0];
            if (previewBody) observer.observe(previewBody, { childList: true, subtree: true });
        }
    });

    elementor.hooks.addFilter('elements/layout/context-menu', (groups, element) => {
        groups.push({
            name: 'sitecore_ai',
            actions: [{
                name: 'ai_agent_task',
                title: 'AI: Optimize This',
                icon: 'eicon-ai',
                callback: () => initAiModal().show()
            }]
        });
        return groups;
    });
