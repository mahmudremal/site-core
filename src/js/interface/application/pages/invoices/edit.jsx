          ...
          <div className="form-wizard-header overflow-x-auto scroll-sm xpo_pb-8 my-32">
            <ul className="list-unstyled form-wizard-list style-three">
              {[1, 2, 3, 4].map((s) => (
                <li
                  key={s}
                  className={`form-wizard-list__item flex xpo_items-center xpo_gap-8 ${step === s ? 'active' : ''}`}
                >
          ...
          {step === 2 && (
            <fieldset className="wizard-fieldset show">
              ...
              <div className="row gy-3">
                ...
                <div className="col-12 flex xpo_justify-between xpo_items-center">
                  <strong>{__('Total')}:</strong>
                  <span className="text-xl fw-bold">{print_money(form.total, form.currency)}</span>
                </div>
                <div className="form-group flex xpo_items-center xpo_justify-content-end xpo_gap-8">
          ...
          {step === 3 && (
            <fieldset className="wizard-fieldset show">
              ...
              <div className="row gy-3">
                ...
                <div className="col-12">
                  ...
                  <ul className="list-group">
                    {form.items.filter(item => item).map((item, i) => (
                      <li key={i} className="list-group-item flex xpo_justify-between">
                ...
                <div className="col-12 flex xpo_justify-between border-top xpo_pt-3 xpo_mt-3">
                  <strong>{__('Total')}</strong>
                  <span>{print_money(form.total, form.currency)}</span>
                </div>
              </div>
              <div className="form-group flex xpo_items-center xpo_justify-content-end xpo_gap-8 xpo_mt-4">
    ...
    <div className="p-4 border rounded shadow-sm bg-white space-y-4">
      <div className="xpo_flex xpo_gap-2">
        ...
        <button
          ...
          className="btn rounded-pill btn-link xpo_text-secondary-light xpo_text-decoration-none radius-8 px-20 py-11 hover:text-white"
        >{__('Copy')}</button>
      </div>

      <form onSubmit={handleShareSubmit} className="space-y-4">
        <label className="block xpo_text-sm font-medium xpo_text-gray-700">{__('Invite via Email')}</label>

        <div className="xpo_flex xpo_flex-wrap xpo_gap-2 xpo_p-2 border rounded bg-gray-50">
          {emails.map(email => (
            <div key={email} className="xpo_flex xpo_items-center bg-gray-200 px-2 py-1 rounded xpo_text-sm">
              {email}
              <button
                type="button"
                onClick={() => setEmails(prev => prev.filter(e => e !== email))}
                className="ml-1 xpo_text-red-600 hover:text-red-800"
              ><X /></button>
            </div>
          ))}
          <input
            ...
            className="xpo_flex-grow border-none focus:ring-0 outline-none xpo_text-sm bg-transparent"
          />
        </div>
        ...
        <textarea
          ...
          className="w-full border rounded xpo_p-2 xpo_text-sm"
        />

        <button
          type="submit"
          disabled={loading}
          className={`flex xpo_items-center xpo_justify-center xpo_gap-2 bg-primary-600 xpo_text-white px-6 py-2 rounded hover:bg-primary-700 ${loading ? 'cursor-not-allowed' : ''}`}
        >
