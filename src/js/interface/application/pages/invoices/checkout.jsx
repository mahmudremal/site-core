...
export default function InvoiceCheckout() {
    ...
    const fetchThings = async () => {
        setLoading(true);
        request(rest_url(`/sitecore/v1/invoice/${invoice_id}`))
        ...
        .catch(err => notify.error(err?.response?.message??err?.message??__('Something went wrong!')));
        // 
        request(rest_url(`/sitecore/v1/payment/gateways`))
            ...
            <div className="col-xxl-9 col-lg-8">
                <div className="card xpo_p-0 radius-12">
                    <div className="card-body px-24 py-32">
                        ...
                        <div className="xpo_flex xpo_items-center xpo_justify-between xpo_mb-24">
                            <div className="xpo_flex xpo_items-center">
                                ...
                                <div className="xpo_flex-grow-1 flex xpo_flex-col">
                                    <h4 className="mb-4">{__('Invoice')}</h4>
                                    <span className="text-md xpo_mb-0 fw-medium xpo_text-neutral-500 block">#{invoice_id}</span>
                                </div>
                            </div>
                        </div>
                        ...
                        {(invoice?.items??[]).map((item, iIndex) => {
                            ...
                            return (
                                <div key={iIndex} className="border radius-12 xpo_p-24">
                                    <h6 className="text-md xpo_mb-16">{pricing?.list_title??__('What’s included')}: {invoice.items.length >= 1 && item.label}</h6>
                            ...
                            <div className="xpo_flex xpo_flex-col xpo_gap-5 xpo_justify-between">
                                <div>
                                    ...
                                    <div className="mt-24 border radius-8 position-relative">
                                        <div className="p-16 flex xpo_items-center border-bottom">
                                            <span className="text-neutral-500 fw-medium xpo_w-76-px border-end">{__("Date")}</span>
                                            <div className="xpo_flex xpo_items-center xpo_justify-between xpo_flex-grow-1 ps-16">
                                        ...
                                        <div className="p-16 flex xpo_items-center">
                                            ...
                                            <div className="xpo_flex xpo_items-center xpo_justify-between xpo_flex-grow-1 ps-16">
                                                <div className="relative select-none">
                                                    <div className="xpo_flex xpo_items-center xpo_gap-8">
                                                        ...
                                                        <div className="xpo_flex-grow-1 flex xpo_flex-col">
                                                            <span className="text-sm xpo_mb-0 fw-medium xpo_text-primary-light block">{gateways[selectedGateWay]?.title??''}</span>
                                                        </div>
                                                    </div>
                                                    <ul className={ `dropdown-menu translate-y-2.5 ${methodOpen && 'show'} ${gateways.length <= 1 && 'hidden'}` }>
                                                        {/* .filter((g, index) => index !== selectedGateWay) */}
                                                        {gateways.map((g, index) => 
                                                            index == selectedGateWay ? <li key={index} className="hidden"></li> : 
                                                            <li
                                                                key={index}
                                                                onClick={() => setSelectedGateWay(index)}
                                                                className="dropdown-item px-2 py-3 cursor-pointer rounded xpo_text-secondary-light bg-hover-neutral-200 xpo_text-hover-neutral-900"
                                                            >
                                                                <div className="xpo_flex xpo_items-center xpo_gap-8">
                                                                    <img src={ g?.icon??'' } alt="" className="w-24-px xpo_h-24-px rounded-circle xpo_flex-shrink-0 overflow-hidden" />
                                                                    <div className="xpo_flex-grow-1 flex xpo_flex-col">
                                                                        <span className="text-sm xpo_mb-0 fw-medium xpo_text-primary-light block">{g?.title??'N/A'}</span>
                                                                    </div>
                                                                </div>
                                                            </li>
                                                        )}
                                                ...
                                                <ChevronDown
                                                    ...
                                                    className={ `text-md xpo_text-neutral-500 xpo_text-hover-primary-600 transition-all duration-300 ease-in-out ${methodOpen && 'rotate-180'} ${gateways.length <= 1 && 'hidden'}` }
                                                />
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <button
                                        ...
                                        className="btn btn-primary xpo_text-sm btn-sm px-12 py-16 xpo_w-100 radius-8 xpo_mt-24 xpo_pb-8 flex xpo_gap-4 xpo_justify-center"
                                    >
                                        {doing && <Loader className="icon xpo_text-xl line-height-1 animate-spin" />}
                                        {__('Pay now')}
                                    </button>
                                </div>
                                <div>
