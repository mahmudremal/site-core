    ...
    return (
        <div className="card xpo_p-0 radius-12">
            <div className="card-header">
                <div className="xpo_flex xpo_items-center xpo_justify-content-between">
                        ...
                        <div className="xpo_flex xpo_items-center xpo_justify-between xpo_gap-4 xpo_w-full">
                            <div className="btn-group radius-8" role="group" aria-label="Default button group">
                                {[
                                    ...
                                    <button key={index} type="button" className={ `btn btn-link xpo_text-secondary-light xpo_text-decoration-none xpo_p-2 hover:xpo_bg-transparent ${filters.store_type == key && 'xpo_text-primary'}` } onClick={() => setFilters(prev => ({...prev, store_type: key}))}>{label}</button>
                                ))}
                            </div>
                    ...
                    <table className="table basic-border-table xpo_mb-0">
                        <thead>
                            <tr>
                                ...
                                <th className="xpo_text-center">{__('Details')}</th>
                            </tr>
                        </thead>
                        <tbody>
                            {loading ? (
                                    ...
                                    <tr>
                                        <td colSpan={4} className="xpo_px-4 xpo_py-6">
                                            <div className="alert alert-warning bg-warning-100 xpo_text-warning-600 border-warning-100 px-24 py-11 xpo_mb-0 fw-semibold xpo_text-lg radius-8" role="alert">
                                                <div className="xpo_flex xpo_items-center xpo_justify-content-between xpo_text-lg">
                                                    {__('No stores found!')} 
                                                </div>
                                                                ...
                                                                <p>
                                                                    ...
                                                                    {store?.plans?.length && (
                                                                        <div>
                                                                            ...
                                                                            <div className="xpo_w-full xpo_flex xpo_gap-2">
                                                    ...
                                                    <button
                                                        ...
                                                        onClick={() => setPopup(
                                                            <div className="xpo_relative xpo_max-w-sm xpo_flex xpo_flex-col xpo_gap-5">
                                                                <h6 class="xpo_text-primary-500 xpo_text-lg fw-semibold">{__('Are you sure you want to delete this store?')}</h6>
            ...
            <div className="card-footer">
                <div className="xpo_flex xpo_items-center xpo_justify-content-between xpo_flex-wrap xpo_gap-2 xpo_mt-24">
                    <span>{sprintf(
                    ...
                    <ul className="pagination xpo_flex xpo_flex-wrap xpo_items-center xpo_gap-2 xpo_justify-content-center">
        ...
        <div className="xpo_max-w-md">
            <form method="post" autoComplete="off" encType="multipart/form-data" onSubmit={(e) => handle_submit(e)}>
                <div className="row gy-3">
                        ...
                        <div className="xpo_flex xpo_flex-wrap xpo_w-full xpo_gap-4">
                            {variants.map(({key: tKey, label: tLabel, color}, i) => (
                                <div key={i} className={ `bg-${color}-50 px-20 py-12 radius-8` }>
                                    <span className={ `form-check checked-${color} xpo_flex xpo_items-center xpo_gap-2` }>
                    ...
                    <button type="submit" className={ `btn rounded-pill radius-8 px-20 py-11 xpo_flex xpo_items-center xpo_gap-3 xpo_justify-center ${loading ? 'btn-secondary-light xpo_text-secondary-light' : 'btn-success-100 xpo_text-success-600'}` } disabled={loading}>
                        {loading ? <Loader className="xpo_animate-spin" /> : <Save />}
                        <span>{__('Submit')}</span>
                    </button>
                </div>
            </form>
...
export const get_site = (site_id) => {
    return new Promise((resolve, reject) => {
        request(rest_url(`/sitecore/v1/stores`), {headers: {'Content-Type': 'application/json'}})
        .then(stores => resolve((stores?.list??stores).find(s => s.id == site_id)))
        .catch(err => reject(err));
    });
}
export const store_rest = (site, url) => {
    if (!site) {return url;}
    ...
    return `${site.store_url}/wp-json/${url}`;
}