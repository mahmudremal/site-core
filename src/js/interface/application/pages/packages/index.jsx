...
export default function Packages({ viewType = 'list' }) {
    ...
    return (
        <div className="card xpo_p-0 radius-12 overflow-hidden">
            <div className="card-header border-bottom bg-base py-16 px-24 flex xpo_justify-between xpo_gap-5">
                <div>
            ...
            <div className="card-body px-4 py-2">
                <div className="justify-content-center">
                    <div className="w-full">
                        <div className="xpo_grid xpo_gap-4 grid-cols-1 sm:xpo_grid-cols-2 lg:xpo_grid-cols-3">
                                ...
                                <div className="pricing-plan-wrapper" key={index}>
                                    ...
                                    <div className="pricing-plan position-relative radius-24 overflow-hidden border bg-lilac-100">
                                        <div className="xpo_flex xpo_items-center xpo_gap-16">
                                            <span className="w-72-px xpo_h-72-px flex xpo_justify-content-center xpo_items-center radius-16 bg-base">
                                        ...
                                        {pack?.pricing?.[plan] && <h3 className="mb-24">{print_money(pack?.pricing?.[plan]??0)} <span className="fw-medium xpo_text-md xpo_text-secondary-light lowercase">/{plan}</span></h3>}
                                        ...
                                        <ul>
                                            {pack.list.map((itemTitle, itemIndex) => 
                                                <li className="xpo_flex xpo_items-center xpo_gap-16 xpo_mb-16" key={itemIndex}>
                                                    <span className="w-24-px xpo_h-24-px flex xpo_justify-content-center xpo_items-center bg-lilac-600 rounded-circle">
            ...
            {paymentLink ? (
                <div className="xpo_flex xpo_flex-col xpo_gap-4">
                    <div className="relative">
                        <img src={receiptBanner} alt={__('Receipt generated')} className="max-h-48" />
                        <div className="absolute xpo_w-full xpo_h-full top-0 left-0"></div>
                    </div>
                    <h6 className="text-center">{__('Payment link')}</h6>
                    <p className="text-center">{__('Click the button below to proceed to payment.')}</p>
                    <a href={paymentLink} target="_blank" rel="noopener noreferrer" className="btn btn-primary xpo_mt-2 animate-bounce xpo_w-full">{__('Proceed to payment')}</a>
                </div>
            ) : (
                <form onSubmit={(e) => handle_package_submit(e, pack, plan)}>
                    <div className="xpo_flex xpo_flex-col xpo_gap-3">
                        <h6>Select a store</h6>
                        <div className="block xpo_mt-6">
                            <ul className="list-group radius-8">
                                {stores.map((store, index) => (
                                    <li key={index} className={ `list-group-item border xpo_text-secondary-light xpo_p-16 xpo_text-white card ${stores.length == (index + 1) ? '' : 'border-bottom-0'}` }>
                                        <div className="form-check checked-warning flex xpo_items-center xpo_gap-2">
                        ...
                        <div className="xpo_flex align-items-start xpo_flex-column xpo_flex-wrap xpo_gap-3 xpo_mt-8">
                            <button className={`btn rounded-pill btn-success-100 xpo_text-success-600 radius-8 px-20 py-11 ${loading ? 'disabled:cursor-progress' : ''}`}>{loading ? __('Generating invoice...') : __('Create invoice')}</button>
                        </div>
                    </div>
                </form>
            )}
