...
export default function MeetsRoom() {
  ...
  useEffect(() => {
    const init = async () => {
      ...
      peerInstance.on('open', id => {
        if (room_id !== id) {
          const call = peerInstance.call(room_id, stream);
  ...
  return (
    <div className="xpo_bg-gradient-to-br xpo_from-[#0f172a] xpo_to-[#1e293b] xpo_min-h-screen xpo_text-white xpo_relative">
      ...
      <div className="xpo_fixed xpo_bottom-4 xpo_left-0 xpo_right-0 xpo_flex xpo_justify-center xpo_space-x-4">
        <button onClick={e => navigate(home_route('meets'))} className="xpo_bg-red-600 xpo_p-3 xpo_rounded-full xpo_text-white"><PhoneOff /></button>
      ...
      <div className="xpo_absolute xpo_bottom-20 xpo_left-8 xpo_bg-white xpo_text-black xpo_p-4 xpo_rounded-lg xpo_shadow-xl">
        <p className="xpo_font-semibold">{__("Your meeting's ready")}</p>
        <input readOnly className="xpo_mt-2 xpo_p-2 xpo_border xpo_rounded w-full" value={`${window.location.protocol}//${window.location.host}${home_route('meets', room_id)}`} />
        <p className="xpo_text-xs xpo_text-gray-500 xpo_mt-2">{__('People need your permission to join')}</p>
      </div>
    </div>
  );
};
