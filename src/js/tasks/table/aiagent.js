...
class AIAgent {
  ...
  constructor() {
    this.baseUrl = `https://${location.host}/wp-json/sitecore/v1`;
    this.ollamaBase = "http://localhost:11434";
    this.ollamaUrl = `${this.ollamaBase}/api/generate`;
                      ...
                      try {
                        ...
                        if (data.response) {
                          ...
                          if (args.onProgress) {
                            ...
                            const progress = data.done
                              ? 100
                              : Math.min(95, fullResult.length / 5);
  ...
  async runSingleTask(args = {}) {
    try {
      ...
      if (args.onLog)
        args.onLog(
          `Processing task ${task.id} of type ${task.task_type}`,
          "info",
        );
      ...
      if (args.onLog)
        args.onLog(`Task ${task.id} processing completed`, "success");
      ...
      if (args.onLog)
        args.onLog(`Error in runSingleTask: ${error.message}`, "error");
      throw error;
    }
  }
  ...
  async startContinuousMode(args = {}) {
    ...
    while (this.isRunning && !this.shouldStop) {
      try {
        ...
        if (args.onLog)
          args.onLog(`Error in agent loop: ${error.message}`, "error");
