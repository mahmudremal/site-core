...
export const ProductCard3 = ({ product, viewMode = 'card' }) => {
    const { __ } = useLocale();
    const { money } = useCurrency();
    return (
        <div key={product.id} className="xpo_group xpo_bg-scwhite/70 xpo_border xpo_border-gray-200 xpo_rounded-xl xpo_p-4 hover:xpo_shadow-lg xpo_transition-all xpo_duration-300 hover:xpo_border-gray-300">
            <div className="xpo_relative xpo_mb-4">
                <img alt={product.name} src={product.image} className="xpo_w-full xpo_h-48 xpo_object-cover xpo_rounded-lg" />
                ...
                <button className="xpo_absolute xpo_top-3 xpo_right-3 xpo_bg-scwhite/70 xpo_p-2 xpo_rounded-full xpo_shadow-md xpo_opacity-0 xpo_group-hover:opacity-100 xpo_transition-opacity hover:xpo_bg-gray-50">
                    <Heart className="xpo_w-4 xpo_h-4 xpo_text-gray-600 hover:xpo_text-red-500" />
                </button>
                <div className="xpo_absolute xpo_top-3 xpo_right-2 xpo_bg-red-500 xpo_text-white xpo_text-xs xpo_font-bold xpo_px-2 xpo_py-1 xpo_rounded-full">
                    {Math.round(((product.originalPrice - product?.metadata?.price) / product.originalPrice) * 100)}% OFF
                </div>
            </div>
            
            <h3 className="xpo_font-medium xpo_text-gray-900 xpo_mb-2 xpo_group-hover:text-scaccent-600 xpo_transition-colors xpo_line-clamp-2">
                {product.name}
            </h3>
            ...
            <div className="xpo_flex xpo_items-center xpo_mb-3">
                ...
                <span className="xpo_text-sm xpo_text-gray-600">
                    {sprintf(__('%f (%d)', 'site-core'), product.rating, product.reviews)}
                </span>
            </div>
            ...
            <div className="xpo_flex xpo_items-center xpo_justify-between xpo_mb-4">
                ...
                <span className="xpo_text-sm xpo_text-green-600 xpo_font-medium">
                    {sprintf(__('Save %s', 'site-core'), money(product.originalPrice - product?.metadata?.price, product?.metadata?.currency))}
                </span>
            </div>
            
            <div className="xpo_flex xpo_gap-2">
                <button className="xpo_flex-1 xpo_bg-gray-900 xpo_text-white xpo_py-2 xpo_px-4 xpo_rounded-lg xpo_font-medium hover:xpo_bg-gray-800 xpo_transition-colors xpo_flex xpo_items-center xpo_justify-center xpo_gap-2">
                    <ShoppingBag className="xpo_w-4 xpo_h-4" />
                    {__('Add to Cart', 'site-core')}
                </button>
                <button className="xpo_p-2 xpo_border xpo_border-gray-200 xpo_rounded-lg hover:xpo_bg-gray-50 xpo_transition-colors">
                    <Plus className="xpo_w-4 xpo_h-4 xpo_text-gray-600" />
                </button>
            </div>
        </div>
    )
