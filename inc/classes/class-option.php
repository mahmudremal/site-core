...
class Option {
  ...
  private $file;
	...
	protected function setup_hooks() {
		$this->file = WP_SITECORE__FILE__;
		$this->plugin_slug = 'site-core';
		$this->plugin_name = 'site-core';
		$this->textdomain = 'site-core'; // str_replace('_', '-', $plugin_slug);
		// Initialise settings
		...
		add_filter('pm_project/system/getoption', [$this, 'getOption'], 10, 2);
		add_filter('pm_project/system/isactive', [$this, 'isActive'], 10, 1);
	}
	/**
	 * Initialise settings
	 * @return void
	...
	public function init() {
		$this->general = (object) wp_parse_args(apply_filters('pm_project/settings/general', []), [
			'page_title'					=> __('Indepth Settings', 'site-core'),
			'menu_title'					=> __('Settings', 'site-core'),
			'role'							=> 'manage_options',
			'slug'							=> $this->plugin_slug,
			'page_header'					=> __('Application in depth configuration screen.', 'site-core'),
			'page_subheader'				=> __('....', 'site-core'),
			'no_password'					=> __('A password is required.', 'site-core'),
		]);
	}
	...
	public function add_settings_link($links) {
		$settings_link = '<a href="options-general.php?page='.$this->general->slug.'">' . __('Settings', 'site-core') . '</a>';
		// array_push($links, $settings_link);
		$links[] = $settings_link;
		$links[] = '<a href="https://www.mahmudremal.com/" target="_blank">' . __('Developer Support', 'site-core') . '</a>';
		return $links;
	}
	/**
	...
	public function getOption($option, $default) {
		return isset(WP_SITECORE_OPTIONS[$option])?WP_SITECORE_OPTIONS[$option]:$default;
	}
	/**
	...
	public function isActive($option) {
		return (isset(WP_SITECORE_OPTIONS[$option]) && WP_SITECORE_OPTIONS[$option] == 'on');
	}
}