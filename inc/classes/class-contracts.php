...
class Contracts {
    ...
    public function select_package_shortcode($atts) {
        ...
        $output = empty($atts['hide_button']) ? sprintf(
            '<button class="select-package-button %s" data-tax_id="%s">%s</button>',
            esc_attr($atts['class']), esc_attr($atts['tax_id']), esc_html($atts['button_text'])
        ) : '';
        wp_enqueue_style('site-core');
        wp_enqueue_script('site-core');
        return $output;
    ...
    public function settings($args) {
        $args['services'] = [
            'title'                         => __('Agreements', 'site-core'),
			'description'					=> sprintf(__('Service and Service deal maker configuration. Use this shortcode for the action button %s', 'site-core'), '<code>[select-package hide_button=0 button_text="Get Started" tax_id="null" class=""]</code>'),
			'fields'						=> [
				[
